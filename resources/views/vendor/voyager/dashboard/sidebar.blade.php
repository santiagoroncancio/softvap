<div class="side-menu sidebar-inverse">
    <nav class="navbar navbar-default" role="navigation">
        ...
        <div id="adminmenu">
            ...
            @if (Auth::user()->roles->contains(function ($valor, $clave) {
            return in_array($valor['name'], ['admin']);
            }))
            <admin-menu :items="{{ menu('admin', '_json') }}"></admin-menu>
            @elseif (Auth::user()->roles->contains(function ($valor, $clave) {
            return in_array($valor['name'], ['teacher']);
            }))
            {!! menu('profesor', 'layouts.menu') !!}
            @else
            {!! menu('estudiante', 'layouts.menu') !!}
            @endif
        </div>
    </nav>
</div>